...
#undef S_ISDIR
#define S_ISDIR(st_mode) ((st_mode).fattrib & AM_DIR)
#undef S_ISREG
#define S_ISREG(st_mode) ((st_mode).fattrib & AM_ARC)

FATFS *mongoose_vfs; // import fs var

#define fileno(fp) ((((FIL *)fp)->obj).id)
...
#define remove mg_f_unlink
int mg_f_unlink(const char *filename)
{
    return (mongoose_vfs) ? ((FR_OK == f_unlink(mongoose_vfs, filename)) ? 0 : -1) : -1;
}

#define rename mg_f_rename
int mg_f_rename(const char *old_filename, const char *new_filename)
{
    return (mongoose_vfs) ? ((FR_OK == f_rename(mongoose_vfs, old_filename, new_filename)) ? 0 : -1) : -1;
}

// #define fseek mg_flseek
#endif 
...
MG_INTERNAL void mg_find_index_file(const char *path, const char *list,
    ...
    LOG(LL_DEBUG, ("mg_find_index_file path %s\n", path));
  struct mg_str vec;
  ...
  *index_file = NULL;
  ...
  while ((list = mg_next_comma_list_entry(list, &vec, NULL)) != NULL) {
    ...
    *index_file = (char *) MG_REALLOC(*index_file, len);
    ...
    LOG(LL_DEBUG, ("%s%c%.*s", path, DIRSEP, (int) vec.len, vec.p));
...
MG_INTERNAL void mg_send_http_file(struct mg_connection *nc, char *path,
  ...
  if (is_dav && opts->dav_document_root == NULL) {
    ...
    LOG(LL_DEBUG, ("test mg_http_serve_file2"));
...
MG_INTERNAL void mg_handle_move(struct mg_connection *c,
  ...
  if (dest == NULL) {
    ...
    if (p != NULL && p[1] == '/' &&
      ...
      LOG(LL_DEBUG, ("\npath %s mg_handle_move %s%.*s\n", path, opts->dav_document_root, (int) (dest->p + dest->len - p), p));
...
static int mg_create_itermediate_directories(const char *path) {
  const char *s;
  LOG(LL_DEBUG, ("mg_create_itermediate_directories path %s\n", path));
...
MG_INTERNAL void mg_handle_put(struct mg_connection *nc, const char *path,
  ...
  if ((rc = mg_create_itermediate_directories(path)) == 0) {
    ...
    LOG(LL_DEBUG, ("file.fp %p", pd->file.fp));
    ...
    if (range_hdr != NULL &&
      ...
      status_code = 206;
      LOG(LL_DEBUG, ("fseeko r1 %lld", r1));
      fseeko(pd->file.fp, r1, SEEK_SET);
      pd->file.cl = r2 > r1 ? r2 - r1 + 1 : pd->file.cl - r1;
    }

    LOG(LL_DEBUG, ("range_hdr %p", range_hdr));
...
int mg_stat(const char *path, cs_stat_t *st) {
  ...
  return _wstati64(wpath, st);
    ...
    {
        FRESULT res = f_stat(mongoose_vfs, path, &(st->st_mode));
        LOG(LL_DEBUG, ("f_stat res %d", res));
        if (res == FR_OK) 
        {
            if (!(st->st_mode.fattrib & AM_DIR)) 
            {
        ...
        else
        {
            ...
            LOG(LL_DEBUG, ("opendir %p", dir));
            if(dir) 
            {
                FILINFO fno;
                FRESULT res = f_readdir(dir, &fno);
                LOG(LL_DEBUG, ("f_readdir %d", res));
                if (res == FR_OK && fno.fname[0] != 0) 
                {
                    if (fno.fattrib & AM_DIR)
                    {
	                    st->st_mode.fattrib = AM_DIR;
                        LOG(LL_DEBUG, ("dir"));
                    } 
                    else 
                    {  
                        FIL fp;
                        if (FR_OK == f_open(mongoose_vfs, &fp, path, FA_OPEN_EXISTING)) 
                        {
                            st->st_mode.fattrib = AM_ARC;
    ...
    {
        DIR * d = opendir(path);
        result = (d) ? (st->st_mode = (st->st_mode & (~_IFMT)) | _IFDIR, closedir(d), 0) : (-1);
    }
    return result;
    #endif
#else
...
FILE *mg_fopen(const char *path, const char *mode_s) {
#ifdef _WIN32
  ...
  return _wfopen(wpath, wmode);
    ...
    if(NULL != mongoose_vfs) 
    {
        ...
        if(NULL != fp)
        {
            FRESULT res = f_open(mongoose_vfs, fp, path, mode);
            LOG(LL_DEBUG, ("f_open %d mode %d", res, mode));
            if (res == FR_OK)
            {
...
#ifdef ESP32_FATFS
    LOG(LL_DEBUG, ("ptr %p f %p size %d ", ptr, f, size * count));
    FIL *fp = (FIL *)f;
    UINT read_size = 0;
    int cnt = f_read(fp, ptr, size * count, &read_size);
    if (cnt == FR_OK) 
    {
        LOG(LL_DEBUG, ("f_read size %d read_size %d", size * count, read_size));
        return read_size;
    }
    return 0;
#endif
...
#ifdef ESP32_FATFS
    LOG(LL_DEBUG, ("ptr %p f %p size %d count %d", ptr, f, size, count));
    FIL *fp = (FIL *)f;
    UINT write_size = 0;
    int cnt = f_write(fp, ptr, size * count, &write_size);
    if (cnt == FR_OK) 
    {
        LOG(LL_DEBUG, ("mg_fwrite size %d write_size %d", size * count, write_size));
        return write_size;
    }
    return 0;
#endif
